...
class DetailController extends CommonController{
	...
	public function index(){
		...
		//查询日期的总营业额
		$totalAmout_searchDay = $totalAmount_isCashSearchDay+$totalAmount_isAlipaySearchDay+$totalAmount_isWeChatSearchDay+$totalAmount_isRemainderSearchDay; // 新增余额
	...
	public function getOneYearTurnover_whithSearch($searchYear,$storeRange){
		...
		$pay_type = array("in","0,1,2,4");  // 新增余额
		...
		for($i=0;$i<=11;$i++){
			...
			$everyTotalAmount =  $this->getTurnover_withCondition($everyConditionSet);
	...
	public function getYearList($adminType = 0){
		...
		$conditionSet['pay_type'] = array("in","0,1,2,4");  // 新增余额
	...
	public function getEveryDayTurnover($Date,$pay_type,$storeRange){
		...
		for($i=0;$i<count($everyDayTimeStampArr);$i++){
			...
			$conditionSet = $this->getConditionSet_isTurnover($startTimeStamp, $endTimeStamp, $pay_type, $storeRange);
			$totalAmount_withCondition = $this->getTurnover_withCondition($conditionSet);
