//设置css样式
function setHBillHeight(){
    //设定广告牌图片的高度
    var hBill = $(".img-div");
    var hBillWidth = hBill.css("width");
    hBillWidth = parseInt(hBillWidth);
    console.log(hBillWidth*0.6+"px");
    $(".img-div .bill-img").css("height",hBillWidth*0.6+"px");
}
...
var vm = new Vue({
    data:{
        billBoards:[],
        textCenter:true,
        btnEdit:{btn:true,'btn-primary':true,'btn-sm':true},
        btnFee:{btn:true,'btn-success':true,'btn-sm':true},
        btnDel:{btn:true,'btn-danger':true,'btn-sm':true},
        mac:"",
