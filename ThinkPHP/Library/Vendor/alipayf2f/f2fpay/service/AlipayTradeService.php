...
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'./../model/result/AlipayF2FPayResult.php';    //条码支付应答
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/result/AlipayF2FQueryResult.php';	//查询应答
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/result/AlipayF2FRefundResult.php';	//退款应答
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/result/AlipayF2FPrecreateResult.php';//扫码支付(生成二维码)应答
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/builder/AlipayTradeQueryContentBuilder.php';//查询请求bizContent结构体
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/builder/AlipayTradeCancelContentBuilder.php'; //撤销请求bizContent结构体
...
class AlipayTradeService {
	...
	public function qrPay($req) {
		file_put_contents(__DIR__."/"."logsss.txt",'进来了'."\r\n",FILE_APPEND);
		$bizContent = $req->getBizContent();
		//$this->writeLog($bizContent);
	...
	private function aopclientRequestExecute($request, $token = NULL, $appAuthToken = NULL) {
		...
		$aop = new AopClient();
		$aop->gatewayUrl = $this->gateway_url;			//支付宝网关
		...
		$aop->apiVersion = "1.0";
		$aop->postCharset = $this->charset;		//UTF-8


		$aop->format=$this->format;			//返回数据类型"json"
		// 开启页面信息输出
		$aop->debugInfo=true;
