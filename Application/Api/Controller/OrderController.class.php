...
//                            dump($sum);
    ...
    public function orderSynchronization(){
        ...
        if($this->is_security) {
           ...
           /* [{
                "add_time":"2017-03-27 14:44:29",
                "foods":[{"food_attrs":[ ],
                "food_id":1030,"food_num":1}],
                "order_sn":"CD1CCAE334B21720170327144429",
                "pay_num":"",
                "order_type":1
            }]*/
            ...
            foreach ($orderDataInfo as $key => $value) {
                # pay_num（订单号）不为空，qr_num为空，则现金支付
                # pay_num（订单号）不为空，qr_num（条形码）不为空，则刷卡支付
                // pay_num 不为空
