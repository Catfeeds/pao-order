...
//                session("num".$order_sn,1);
                ...
                if($rel !== false){
                    //支付回掉记录
                    file_put_contents(__DIR__."/"."payLog.txt","订单号：".$out_trade_no."|"."时间：".date("Y-m-d H:i:s",time())."|支付方式：支付宝扫码付"."\r\n",FILE_APPEND);
...
//	    $barPayRequestBuilder->setAlipayStoreId($alipayStoreId);
		...
		file_put_contents(__DIR__."/"."logzzzz.txt",'$alipay_config'.var_export($alipay_config,true)."\r\n",FILE_APPEND);
	    ...
	    switch ($barPayResult->getTradeStatus()){
			case "SUCCESS":
	           // print_r($barPayResult->getResponse());
	            ...
	            if($rel !==false){
	                //支付回掉记录
	                file_put_contents(__DIR__."/"."payLog.txt","订单号：".$order_sn."|"."时间：".date("Y-m-d H:i:s",time())."|支付方式：支付宝条码支付"."\r\n",FILE_APPEND);
	                //获取订单信息，判断是否要推送到展示餐牌号展示页面
	                $orderInfo = $orderModel->where($o_condition)->field("table_num,desk_code,restaurant_id")->find();
	                $restaurantModel = D("Restaurant");
	                $rr_condition['restaurant_id'] = $orderInfo['restaurant_id'];
