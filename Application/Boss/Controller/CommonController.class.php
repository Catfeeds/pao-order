...
//		$conditionSet['pay_type'] = array("in",'0,1,2');		//第三个条件：支付方式
		$conditionSet['pay_type'] = array("in",'0,1,2,4');		//第三个条件：支付方式      新增一个余额支付
		$conditionSet['pay_time'] = array("between",array($startTimeStamp,$endTimeStamp));	//第四个条件：时间范围
		$conditionSet['order_type'] = array('in','1,2,3');
		return $conditionSet;
	}
	
	//营业额统计条件集
	public function getConditionSet_isTurnover($startTimeStamp,$endTimeStamp,$pay_type,$storeRange){
		$conditionSet['restaurant_id'] = $storeRange;			//第一个条件：店铺ID集
		$conditionSet['order_status'] = array('neq',0);						//第二个条件：订单状态
